<template>
  <section class="relative">
    ...
    v-if="project?.image"
    aria-hidden="true"
    class="pointer-events-none absolute inset-x-0 top-0 h-[vh] sm:h-[30vh] min-h-[120px] sm:min-h-[140px] overflow-hidden"
    >
    <img
        :src="project.image"
        alt=""
...
function smartBack() {
  ...
  if (hasHistory && cameFromSameOrigin && window.history.state?.back) {
    router.back()
  } else {
...
const actionLinks = computed(() => {
  ...
  if (p.github_link) {
    links.push({
      ...
      label: 'GitHub Link',
      href: p.github_link,
      class: 'bg-[#24292e] text-white hover:opacity-90 focus-visible:ring-[#24292e]'
    });
  }
  ...
  if (p.app_link) {
    links.push({
      key: 'app',
      label: 'Direct-App Link',
      href: p.app_link,
      class: 'bg-[#475569] text-white hover:brightness-110 focus-visible:ring-[#475569]' // slate-600
    });
  }
  ...
  if (p.appstore_link) {
    links.push({
      ...
      href: p.appstore_link,
      class: 'bg-[#0a84ff] text-white hover:brightness-110 focus-visible:ring-[#0a84ff]' // Apple blue
    });
  }
  ...
  if (p.playstore_link) {
    links.push({
      ...
      href: p.playstore_link,
      class: 'bg-[#34a853] text-white hover:brightness-110 focus-visible:ring-[#34a853]' // Google green
    });
  }
  ...
  if (p.medium_link) {
    links.push({
      ...
      label: 'Medium Link',
      href: p.medium_link,
      class: 'bg-[#00ab6c] text-white hover:brightness-110 focus-visible:ring-[#00ab6c]'
    });
  }

  if (p.website_reference_link) {
    links.push({
      key: 'website',
      label: 'Website Link',
      href: p.website_reference_link,
      class: 'bg-[#6366f1] text-white hover:brightness-110 focus-visible:ring-[#6366f1]' // indigo-500
    });
  }
  ...
  if (p.slideshow_link) {
    links.push({
      ...
      href: p.slideshow_link,
      class: 'bg-[#f4b400] text-black hover:brightness-95 focus-visible:ring-[#f4b400]' // GSlides yellow
    });
  }
